...
// Current Weather
function displayCurrent(obj, name, units) {
  ...
  let formSpd = Math.round(current.wind_speed);
  let tempSuffix = "\u00B0F";
  let spdSuffix = "m/h";
  if (units === "metric") {
    tempSuffix = "\u00B0C";
    spdSuffix = "km/h";
    formSpd = Math.round(formSpd * 3.6);
  } else {
    tempSuffix = "\u00B0F";
    spdSuffix = "m/h";
  }
  ...
  temp.textContent = formTemp + tempSuffix;
  feelsLike.textContent = formFeels + tempSuffix;
  humidity.textContent = current.humidity + "%";
  windSpd.textContent = formSpd + spdSuffix;
  ...
  sunrise.textContent = formatTime(
    getTime(current.sunrise, obj.timezone_offset)
  );
...
function displayHourly(obj, units) {
  ...
  cells.forEach((cell) => {
    ...
    addHourlyData(index, obj, cell, units);
  });
}
...
function addHourlyData(index, obj, cell, units) {
  ...
  children.item(2).textContent = formedTemp + tempSuffix;
...
export { displayCurrent, displayHourly, displayDaily };
